...
def load_oxford_flowers102(
    format: str = "numpy", imsize: int = 96, fine: bool = False
) -> Tuple[
    Dict[str, List],  # train_data
    Dict[str, List],  # validation_data
    Dict[str, List],  # test_data
    ...
    numpy_save = "data/oxford_flowers102_%dx%d.data" % (imsize, imsize)
    ...
    if format == "numpy" and os.path.isfile(numpy_save):
        with gzip.open(numpy_save) as f:
            (
                oxford_flowers102_train,
                oxford_flowers102_validation,
                oxford_flowers102_test,
                class_names,
            ) = pickle.load(f)

        oxford_flowers102_train = labelling_select(
            oxford_flowers102_train, k_keep, k_remove
        )
        oxford_flowers102_validation = labelling_select(
            oxford_flowers102_validation, k_keep, k_remove
        )
        oxford_flowers102_test = labelling_select(
            oxford_flowers102_test, k_keep, k_remove
        )
    ...
    oxford_flowers102_train = pandas_numpy_to_data_dict(
        oxford_flowers102_train, 6149, fine_to_group, imsize
    )
    print("Converting validation images to %dx%d numpy format..." % (imsize, imsize))
    oxford_flowers102_validation = pandas_numpy_to_data_dict(
        oxford_flowers102_validation, 1020, fine_to_group, imsize
    )
    print("Converting test images to %dx%d numpy format..." % (imsize, imsize))
    oxford_flowers102_test = pandas_numpy_to_data_dict(
        oxford_flowers102_test, 1020, fine_to_group, imsize
    )
...
if __name__ == "__main__":
    ...
    show_methods.show_data_images(
        images=train_data["images"][:16],
        labels=train_data["labels"][:16],
