...
def flowers_trained_cnn(
    ...
    x_train = augment_data_with_noise(x_train)
    y_hat_train = x_train
    x_test = test_data["images"] / 255
    x_test = augment_data_with_noise(x_test)
    y_hat_test = x_test
    x_validation = validation_data["images"] / 255
    x_validation = augment_data_with_noise(x_validation)
    ...
    if load_from_file and os.path.isfile(net_save_name):
        ...
        x_train, y_hat_train = apply_gaussian_noise_with_random_std(x_train, (0.0, 2.0))
        ...
        save_images_to_jpg(
            result_array, f"./{IMAGE_SIZE}/images_aug/training_one_fit.jpg"
        )
    ...
    try:
        for j in range(10, 0, -1):
            ...
            result_array = np.concatenate(result, axis=0)
            save_images_to_jpg(
                result_array, f"./{IMAGE_SIZE}/images_aug/generated_image_{j}.jpg"
            )
